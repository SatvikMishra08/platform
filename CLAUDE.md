...
- **`model/`** — Wire-protocol types (`Reply`, `BasicReply`, `AutoForwardReply`, `DialogueStep`, `Statement`, `Segment`, `Action`, `OngoingDialogue`, `ServerInfo`, `User`, `Variable`); only `Action`/`Segment` currently have a `fromJSON` parser (see [#232](https://github.com/dialoguebranch/platform/issues/232)).
